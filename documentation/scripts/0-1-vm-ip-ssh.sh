...
# Function: derive .1 gateway from an IPv4 address
gateway_for_ip() {
    local ip="$1"
...
# Derive gateway addresses from target static IPs
GATEWAY="$(gateway_for_ip "$MASTER_IP")"   || { eko RED "Failed deriving gateway"; exit 1; }
...
# set static IPs (dynamic gateway)
